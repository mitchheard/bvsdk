// TODO: [mheard] Modify submission fields, statistics, product (first below) and test
	...
	models : {
		...
		catalog : {
			...
			product : {
				...
				// TODO: Question and Answers need to be in "stats" or should we add another function parameter to pass?
		...
		submission : {
			...
			question : {
				...
				getForm : function (productID, container, callBack, options) {
					...
					var apiCall = bvsdk.models.submission.question.apiCall (settings);
					var urlString = apiCall["url"];
					var paramObject = $bvsdk.param(apiCall["params"]);
					$bvsdk.ajax({
						type: "GET",
				...
				// posts question submission form
				postForm : function (productID, container, callBack, options) {
					var settings = $bvsdk.extend(true, {
						"Parameters":{
							"productid":productID
				...
				// TODO: need to update/remove fields for Questions
				...
				apiCall : function (options) {
					...
					// set URL base for API call
					var url = bvsdk.models.init.locationProtocol + defaultSettings["URL"]["baseurl"] + "data/" + "submitquestion." + defaultSettings["URL"]["format"];
			...
			answer : {
				...
				getForm : function (questionID, container, callBack, options) {
					...
					var apiCall = bvsdk.models.submission.answer.apiCall (settings);
					var urlString = apiCall["url"];
					var paramObject = $bvsdk.param(apiCall["params"]);
					$bvsdk.ajax({
						type: "GET",
				...
				// TODO: Update/remove fileds for answers
				...
				apiCall : function (options) {
					...
					// set URL base for API call
					var url = bvsdk.models.init.locationProtocol + defaultSettings["URL"]["baseurl"] + "data/" + "submitanswer." + defaultSettings["URL"]["format"];
		...
		// TODO: statistics for Question & Answers?
		...
		ugc : {
			...
			question : {
				...
				getUGC : function (productID, container, callBack, options) {
					...
					var apiCall = bvsdk.models.ugc.question.apiCall(settings);
					var urlString = apiCall["url"];
					var paramObject = apiCall["params"];
					var paramString = bvsdk.models.general.returnAPIParametersString (apiCall["params"]);
					$bvsdk.ajax({
				...
				apiCall : function (options) {
					...
					var defaultSettings = $bvsdk.extend(true, {
						...
						"Parameters":{
							...
							"filter_[TYPE]":null, // Filtering option for included nested content. TYPE can be any included nested content. i.e. Comments for Reviews.
							"include":null, // Related subjects to be included (e.g. Products, Categories, Authors, or Comments).
							// TODO: does limitQuestions work?
							"limit":bvsdk.models.config.apiDefaults["limitQuestions"], // Max number of records returned. An error is returned if the value passed exceeds 100.
							"limit_[TYPE]":null, // Limit option for the nested content type returned. TYPE can be any nested content. i.e. Comments for Reviews. An error is returned if the value passed exceeds 20.
							"locale":null, // Locale to display Labels, Configuration, Product Attributes and Category Attributes in. The default value is the locale defined in the display associated with the API key.
							"offset":bvsdk.models.config.apiDefaults["offset"], // Index at which to return results. By default, indexing begins at 0 when you issue a query. Using Limit=100, Offset=0 returns results 0-99. When changing this to Offset=1, results 1-100 are returned.
							"passkey":bvsdk.models.config.apiDefaults["passkey"], //API key is required to authenticate API user and check permission to access particular client's data.
					...
					// set URL base for API call
					var url = bvsdk.models.init.locationProtocol + defaultSettings["URL"]["baseurl"] + "data/" + "questions." + defaultSettings["URL"]["format"];
			...
			answer : {
				...
				// get comments - set questionID to null to return all
				getUGC : function (questionID, container, callBack, options) {
					var settings = $bvsdk.extend(true, {
						"Parameters":{
							"filter":{
								"questionid":questionID
					...
					var apiCall = bvsdk.models.ugc.answer.apiCall (settings);
					var urlString = apiCall["url"];
					var paramObject = apiCall["params"];
					var paramString = bvsdk.models.general.returnAPIParametersString (apiCall["params"]);
					$bvsdk.ajax({
				...
				apiCall : function (options) {
					...
					var defaultSettings = $bvsdk.extend(true, {
						...
						"Parameters":{
							...
							"include":null, // Related subjects to be included (e.g. Products, Categories, Authors, or Comments).
							// TODO: does limitAnswers work?
							"limit":bvsdk.models.config.apiDefaults["limitAnswers"], // Max number of records returned. An error is returned if the value passed exceeds 100.
							"locale":null, // Locale to display Labels, Configuration, Product Attributes and Category Attributes in. The default value is the locale defined in the display associated with the API key.
							"offset":bvsdk.models.config.apiDefaults["offset"], // Index at which to return results. By default, indexing begins at 0 when you issue a query. Using Limit=100, Offset=0 returns results 0-99. When changing this to Offset=1, results 1-100 are returned.
							"passkey":bvsdk.models.config.apiDefaults["passkey"], //API key is required to authenticate API user and check permission to access particular client's data.
							"search":null, // Full-text search string used to find UGC. For more information about what fields are searched by default, see the API Basics page.
					...
					// set URL base for API call
					var url = bvsdk.models.init.locationProtocol + defaultSettings["URL"]["baseurl"] + "data/" + "answers." + defaultSettings["URL"]["format"];
